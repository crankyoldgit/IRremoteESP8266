...
void IRDaikin160::stateReset() {
  ...
  remote_state[13] = 0x51;
  //remote_state[16] = 0x1E;
  //remote_state[17] = 0x0F;
...
void IRDaikin160::setTemp(const uint8_t temp) {
  uint8_t degrees = std::max(temp, kDaikinMinTemp);
  //degrees = std::min(degrees, kDaikinMaxTemp) - kDaikinMinTemp;
...
uint8_t IRDaikin160::getTemp(void) {
  return (remote_state[kDaikin160ByteTemp] & kDaikin160MaskTemp) +kDaikinMinTemp;
}
...
void IRDaikin160::setSwingVertical(const bool on) {
  if (on)
    ...
    remote_state[kDaikin160ByteSwingV] &= 0x31; //kDaikin160MaskSwingV; // removed '~' symbol
}
...
uint8_t IRDaikin160::getSwingVertical() { return remote_state[13] & 0x0F; }

*/
//void IRDaikin160::setSwingHorizontal(const bool on) {
//  if (on)
//    remote_state[kDaikin160ByteSwingH] |= kDaikin160MaskSwingH;
//  else
//    remote_state[kDaikin160ByteSwingH] &= ~kDaikin160MaskSwingH;
//}

//bool IRDaikin160::getSwingHorizontal(void) {
//  return remote_state[kDaikin160ByteSwingH] & kDaikin160MaskSwingH;
//}

// This is a horrible hack till someone works out the quiet mode bit.
//void IRDaikin160::setQuiet(const bool on) {
//  if (on)
//    this->setFan(kDaikinFanQuiet);
//  else if (this->getFan() == kDaikinFanQuiet)
//   this->setFan(kDaikinFanAuto);
...
// This is a horrible hack till someone works out the quiet mode bit.
//bool IRDaikin160::getQuiet(void) {
//  return this->getFan() == kDaikinFanQuiet;
//}
...
String IRDaikin160::toString() {
  String result = "";
#else   // ARDUINO
std::string IRDaikin160::toString() {
  std::string result = "";
#endif  // ARDUINO
  result += F("Power: ");
  if (this->getPower())
    ...
    for (; offset <= results->rawlen - 16 && i < pos;
      ...
      data_result =
          matchData(&(results->rawbuf[offset]), 8, kDaikin160BitMark,
                    ...
                    kDaikin160ZeroSpace, kTolerance, kMarkExcess, false);
      if (data_result.success == false) break;  // Fail
      results->state[i] = (uint8_t)data_result.data;
    }

    // Section Footer
    if (!matchMark(results->rawbuf[offset++], kDaikin160BitMark)) return false;
