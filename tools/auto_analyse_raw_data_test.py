...
class TestAutoAnalyseRawData(unittest.TestCase):
  ...
  def test_parse_and_report(self):
    ...
    self.assertEqual(
        ...
        '                      kFOOBitMark, kFOOOneSpace,\n'
        '                      kFOOBitMark, kFOOZeroSpace,\n'
        '                      kFOOBitMark, kFOOHdrSpace, true);\n'
        '  if (used == 0) return false;  // We failed to find any data.\n'
        '  offset += used;  // Adjust for how much of the message we read.\n'
        '  pos += 1;  // Adjust by how many bytes of data we read\n'
        '\n'
        '  // Data Section #2\n'
        '  // e.g.\n'
        '  //   bits = 8; bytes = 1;\n'
        '  //   *(results->state + pos) = {0x01};\n'
        '  used = matchGeneric(results->rawbuf + offset, results->state + pos,'
        '\n'
        '                      results->rawlen - offset, 8,\n'
        '                      0, kFOOHdrSpace,\n'
  ...
  def test_unusual_gaps(self):
    ...
    self.assertEqual(
        ...
        'void IRsend::sendFOO(uint8_t data[], uint16_t nbytes,'
        ' uint16_t repeat) {\n'
