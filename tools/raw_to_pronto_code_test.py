...
class TestRawToPronto(unittest.TestCase):
  ...
  def test_parse_and_report_at_38000(self):
    ...
    pronto.parse_and_report(input_str, 38000, False, output)
    self.assertEqual(
        output.getvalue(),
        "Pronto code = '0000 006D 0007 0000 02FB 0309 023D 048E 02FB 0309 023D'"
        "\n")
  ...
  def test_parse_and_report_at_36000(self):
    ...
    input_str = """
        uint16_t rawData[7] = {
            20100, 20472, 15092, 30704, 20102, 20472, 15086};"""
    pronto.parse_and_report(input_str, 36000, False, output)
    self.assertEqual(
        output.getvalue(),
        "Pronto code = '0000 0073 0007 0000 02D3 02E0 021F 0451 02D3 02E0 021F"
        "'\n")
  ...
  def test_parse_and_report_at_57600(self):
    ...
    input_str = """
      uint16_t rawData[7] = {
          20100, 20472, 15092, 30704, 20102, 20472, 15086};"""
    pronto.parse_and_report(input_str, 57600, False, output)
    self.assertEqual(
        output.getvalue(),
        "Pronto code = '0000 0047 0007 0000 0485 049B 0365 06E8 0485 049B 0364'"
        "\n")
