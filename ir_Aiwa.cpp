...
void  IRsend::sendAiwaRCT501 (int code)
{
	...
	for (unsigned long  mask = 1UL << (26 - 1);  mask;  mask >>= 1) {
		...
		space_encode(pre & mask, AIWA_RC_T501_ONE_SPACE,AIWA_RC_T501_ZERO_SPACE) ;
	}
...
//    I suspect TOPBIT should be changed to 0x00008000
	...
	for (int  i = 0;  i < 15;  i++) {
		mark(AIWA_RC_T501_BIT_MARK);
		space_encode(code & 0x80000000, AIWA_RC_T501_ONE_SPACE, AIWA_RC_T501_ZERO_SPACE) ;
		code <<= 1;
	}
...
bool  IRrecv::decodeAiwaRCT501 (decode_results *results)
{
	...
	// Check SIZE
	if (irparams.rawlen < 2 * (AIWA_RC_T501_SUM_BITS) + 3 + OFFSET_START)  return false ;
	...
	while(offset < irparams.rawlen - 4) {
		if (!MATCH_MARK(results->rawbuf[offset++], AIWA_RC_T501_BIT_MARK))  return false ;
		// ONE & ZERO
		if (!space_decode(data,results->rawbuf[offset++], AIWA_RC_T501_ONE_SPACE, AIWA_RC_T501_ZERO_SPACE)) break ;  // End of one & zero detected
	}
        int bits =0; 
	bits =(offset - OFFSET_START) / 2;
